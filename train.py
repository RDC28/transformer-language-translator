...
def get_ds(config):
    ...
    train_dataloader = DataLoader(train_ds, batch_size=config['batch_size'], shuffle=True,num_workers=0) # Windows + num_workers>0 often buggy without strict guards, set 0 for safety or 2 if guarded
    # Windows typically allows num_workers=0 for stability in interactive scripts, 
    # but we can try 2 if run from __main__. Let's stick to 2 and hope user runs properly.
    # Actually, let's keep it safe with 0 for now as 'num_workers' bugs on Windows are annoying.
    # Speedup mainly needs to come from Model Size reduction.
